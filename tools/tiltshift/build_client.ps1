...
try {
    ...
    & $MsBuild bs2_tiltshift.sln /m:2 /p:Configuration=Release /p:Platform=Win32 /p:WindowsTargetPlatformVersion=10.0 /verbosity:minimal
    if ($LASTEXITCODE -ne 0) { throw 'C++ build failed. Do not deploy a partial build.' }
